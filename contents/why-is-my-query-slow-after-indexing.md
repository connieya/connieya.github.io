...
상품 데이터 50만 건, 상품 좋아요 데이터 10만 건을 적재하고
상품 목록 조회 API 테스트를 진행했다.

API의 주요 기능은 가격, 출시일, 좋아요 수로 상품을 정렬하여 페이징으로 반환하는 것이다.

API의 성능을 확인하고자 k6로 부하 테스트를 진행했는데, 평균 응답 시간이 10초를 훌쩍 넘기는 믿을 수 없는 결과가 나왔다.
...
쿼리 시간은 여전히 5초대에 머물렀다. GROUP BY라는 거대한 벽이 ORDER BY를 위한 인덱스 활용을 원천적으로 차단하고 있었던 것이다.

## 서브쿼리로의 전환 , JOIN에 대한 관점 바꾸기
