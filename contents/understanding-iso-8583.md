...
- **요청(xx0x)**: "이렇게 처리해주세요."라고 상대방에게 무언가를 요구하는 메시지입니다. 우리가 분석했던 0800이 바로 여기에 해당합니다.
- **요청에 대한 응답(xx1x)**: 요청을 받은 시스템이 "네 요청을 이렇게 처리했어."라고 결과를 회신해주는 메시지입니다. 0800 요청에 대한 짝꿍인 0810이 바로 이 경우입니다.
...
앞서 ISO 8583은 최대 128개의 데이터 필드를 정의한다고 했습니다. 그렇다면 비트맵은 최소 128개의 자리를 가져야겠죠. 하지만 실제 통신에서는 이를 **64비트 단위(8바이트)** 로 끊어서 표현합니다.

첫 번째 64비트는 필드 1에서 64까지를 나타내고, 필드 1이 켜져 있으면 “추가 비트맵이 뒤따른다”는 뜻으로 해석되어, 두 번째 64비트(65~128번 필드)가 이어집니다.

이렇게 하면 메시지가 항상 128비트로 고정되는 게 아니라, 필요한 경우에만 확장되므로 더 효율적으로 동작할 수 있습니다.

그리고 실제 통신 원문에서는 이 긴 0과 1을 그대로 나열하지 않고, 사람이 보기 쉽도록 16진수로 압축해 표현합니다. 예를 들어, 앞에서 언급한 “필드 7, 11, 24, 39 사용”이라는 비트맵은 16진수로 변환하면 다음과 같은 값이 됩니다.
