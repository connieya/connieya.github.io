...
- **`전략 패턴`** 은 if-else로 얽혀있던 알고리즘들을 **독립적인 객체로 분리**하여 교체 가능한 부품으로 만드는 데 효과적이었습니다. (has-a 관계, 위임)
- **`템플릿 메소드 패턴`** 은 여러 클래스에 걸쳐 중복되던 **처리 흐름의 뼈대를 통일**하고, **변경이 필요한 부분**만 자식에게 맡기는 데 유용했습니다. (is-a 관계, 상속)
